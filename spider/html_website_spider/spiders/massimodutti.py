...
class Massimodutti(CommonSpider):
    ...
    def parse_product_detail(self, response, **kwargs):
        ...
        detail_api_url = f'https://www.massimodutti.com/itxrest/2/catalog/store/34009456/30359502/category/0/product/{product_id}/detail?languageId=-1&appId=1'
        if product_id:
        ...
        for data in media:
            if data.get("colorCode") == color_code:
                ...
                for file in file_data:
                    url = f"{base_url}{path}/{file}16.jpg"
                    images.append(url)
                return images
